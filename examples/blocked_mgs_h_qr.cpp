...
int main(int argc, char** argv) {
  ...
  Hierarchical<double> Q(zeros, N, N, rank, nleaf, admis, nblocks, nblocks);
  Hierarchical<double> R(zeros, N, N, rank, nleaf, admis, nblocks, nblocks);
  timing::stopAndPrint("Init matrix");
  admis = N / nleaf; // Full rank
  timing::start("Dense tree");
  Hierarchical<double> D(laplacend, randx, N, N, rank, nleaf, admis, nblocks, nblocks);
  ...
  print("H-QR Accuracy");
  Hierarchical<double> QR(zeros, N, N, rank, nleaf, admis, nblocks, nblocks);
  gemm(Q, R, QR, 1, 1);
  print("Residual", l2_error(A_copy, QR), false);
  Hierarchical<double> QtQ(zeros, N, N, rank, nleaf, admis, nblocks, nblocks);
