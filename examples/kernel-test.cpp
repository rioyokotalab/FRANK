...
int main(int argc, char** argv) {
  ...
  const int64_t nleaf = argc > 1 ? atoi(argv[1]) : 32;
  const int64_t rank = argc > 2 ? atoi(argv[2]) : 16;
  const int64_t N = argc > 3 ? atoi(argv[3]) : 256;
  ...
  const Hierarchical A(laplacend, randx, N, N, rank, nleaf, admis, nblocks, nblocks);
  ...
  const std::vector<std::vector<double>> randx1{
    get_sorted_random_vector(N),
    get_sorted_random_vector(N)
  };
  const Hierarchical A1(cauchy2d, randx1, N, N, rank, nleaf, admis, nblocks, nblocks);
  write_JSON(A1, std::string("cauchy2d-") + std::to_string(N) + std::string("-") +
             std::to_string(nleaf) + std::string("-") +
             std::to_string(rank) + std::string("-") +
             std::to_string(admis) + std::string(".json"));
