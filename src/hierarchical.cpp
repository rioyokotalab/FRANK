...
namespace hicma {
  ...
  void Hierarchical::blr_col_qr(Hierarchical& Q, Hierarchical& R) {
    ...
    for(int i=0; i<dim[0]; i++) {
      if((*this)(i, 0).is(HICMA_DENSE)) {
        ...
        std::cerr << "(*this)(" << i << ", 0) has bad type!" << std::endl;
        abort();
      }
    }
    Dense DB(B);
  ...
  void Hierarchical::restore_col(const Hierarchical& Sp, const Hierarchical& QL) {
    ...
    for(int i=0; i<dim[0]; i++) {
      if((*this)(i, 0).is(HICMA_HIERARCHICAL)) {
        ...
        assert(Ai.dim[0] == SpCurRowCombined.dim[0]);
        assert(Ai.dim[1] == SpCurRowCombined.dim[1]);
        ...
        Dense SpCurRowCombined(SpCurRow);
        Dense QLi(QL(i, 0));
        assert(QLi.dim[0] == Ai.dim[0]);
        assert(QLi.dim[1] == Ai.rank);
        assert(SpCurRowCombined.dim[0] == Ai.rank);
        assert(SpCurRowCombined.dim[1] == Ai.dim[1]);
        Ai.U = QLi;
  ...
  void Hierarchical::qr(Hierarchical& Q, Hierarchical &R) {
    assert(Q.dim[0] == dim[0]);
    assert(Q.dim[1] == dim[1]);
    assert(R.dim[0] == dim[1]);
    assert(R.dim[1] == dim[1]);
    for(int j=0; j<dim[1]; j++) {
