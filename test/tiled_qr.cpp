...
int main(int argc, char** argv) {
  ...
  float conditionNumber = argc > 4 ? atof(argv[4]) : 1e+0;
  int64_t Nc = N / Nb;
  std::vector<std::vector<float>> randpts;

  Hierarchical A;
  if(matCode == 0) { //Laplace1D
    std::vector<std::vector<float>> randpts{ equallySpacedVector(N, 0.0, 1.0) };
    ...
    std::vector<float> d(N, 0.0); //Singular values to be used
    int64_t mode = 1; //See docs
    Dense DA(N, N);
    latms(dist, iseed, sym, d, mode, conditionNumber, dmax, kl, ku, pack, DA);
    A = split(DA, Nc, Nc, true);
  }
  Hierarchical T(zeros, std::vector<std::vector<float>>(), N, N, 0, Nb, Nc, Nc, Nc);
  Hierarchical Q(identity, std::vector<std::vector<float>>(), N, N, 0, Nb, Nc, Nc, Nc);
